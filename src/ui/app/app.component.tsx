import { type FC, useEffect, Suspense, lazy, useMemo } from 'react';
...
const LandingPage = lazy(() => import('../landing-page').then((module) => ({ default: module.LandingPage })));
...
const ComingSoonPage = lazy(() => import('../coming-soon-page').then((module) => ({ default: module.ComingSoonPage })));
...
// Memoize queryClient creation to prevent re-initialization
const queryClient = new QueryClient({
	defaultOptions: {
		queries: {
			queryKeyHashFn: hashFn,
			staleTime: 60_000, // 1 minute default (increased from 30s)
			gcTime: 300_000, // 5 minutes garbage collection time
					...
					<Route
						...
						element={
							import.meta.env.MODE === 'prod' ? (
								<ComingSoonPage
									title="Profile"
									description="Manage your lending portfolio and settings"
								/>
							) : (
								<MarketsPage />
							)
...
export const App: FC = () => {
	...
	// Memoize environment detection to prevent recalculation on every render
	const { basename } = useMemo(() => {
		...
		const base = isDev ? '/development' : undefined;
	...
	// Environment debug info computed for development visibility
