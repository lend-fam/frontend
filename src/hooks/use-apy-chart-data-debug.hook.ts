...
import { useHistoricalAPYDebug, type TimeRange } from './use-historical-apy-debug.hook';
...
export const useAPYChartDataDebug = ({ cTokenMarket, timeRange, metric }: UseAPYChartDataOptions) => {
	const {
		data: historicalData,
		loading,
		error,
		isMockData,
	} = useHistoricalAPYDebug({
		cTokenMarket,
		timeRange,
	});
